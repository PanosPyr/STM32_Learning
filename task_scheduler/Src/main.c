...
void init_systick_timer(uint32_t tick_hz)
{
	...
	*pSCSR |= (1 << 2); //Set processor clock as the clock source
...
__attribute__((naked)) void init_scheduler_stack(uint32_t sched_top_of_stack)
{
	...
	//__asm volatile("MSR MSP,%0":: "r"(sched_top_of_stack):); //Same functionality as one above
...
__attribute__((naked)) void switch_sp_to_psp(void)
{
	//1. Init PSP with task1 stack start addr

	//Get PSP value of current stack
